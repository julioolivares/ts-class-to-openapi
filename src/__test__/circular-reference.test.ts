...
// Helper function to check if a property uses $ref or is an object type
function assertRefOrObjectType(
  property: any,
  ...
  if (property.$ref) {
    if (expectedRefType) {
      assert.ok(
        property.$ref.includes(expectedRefType),
        `$ref should reference ${expectedRefType}`
      )
    }
    assert.strictEqual(
      property.type,
      undefined,
      'When using $ref, type should not be set'
    )
  } else {
    assert.strictEqual(
      property.type,
      'object',
      `${propertyName} should be object type if not using $ref`
    )
  }
}
...
describe('Circular Reference Tests', () => {
  it('should handle circular references without infinite recursion - CircularUser', () => {
    ...
    // Check that circular reference uses $ref pattern (OpenAPI 3.1 style)
    const roleProperty = result.schema.properties.role
    assert.ok(
      roleProperty.$ref || roleProperty.type === 'object',
      'Role should use $ref or be object type'
    )
  })
  ...
  it('should handle circular references without infinite recursion - CircularRole', () => {
    ...
    assert.ok(result.schema.properties.name, 'Should have name property')
    assert.ok(
      result.schema.properties.assignedBy,
      'Should have assignedBy property'
    )

    // Check that circular reference uses $ref pattern or is object type
    const assignedByProperty = result.schema.properties.assignedBy
    assert.ok(
      assignedByProperty.$ref || assignedByProperty.type === 'object',
      'AssignedBy should use $ref or be object type'
    )
  })
  ...
  it('should not create infinite nested objects in circular references', () => {
    ...
    // The role property should use $ref or be an object but not infinitely nested
    const roleProperty = userResult.schema.properties.role

    if (roleProperty.$ref) {
      // If using $ref, it should reference CircularRole
      assert.ok(
        roleProperty.$ref.includes('CircularRole'),
        '$ref should reference CircularRole'
      )
    } else {
      // If not using $ref, should be object type
      assert.strictEqual(
        roleProperty.type,
        'object',
        'Role should be object type'
      )
      ...
      if (roleProperty.properties) {
        ...
        if (roleProperty.properties.assignedBy) {
          ...
          assert.ok(
            assignedByProperty.$ref || assignedByProperty.type === 'object',
            'Nested circular reference should use $ref or be simple object type'
          )

          // Should not have infinite nesting - no properties.assignedBy.properties.role.properties...
          assert.ok(
            !roleProperty.properties.assignedBy.properties?.role?.properties
              ?.assignedBy,
      ...
      if (roleProperty.properties && roleProperty.properties.assignedBy) {
        ...
        if (assignedByProperty.description) {
          assert.ok(
            assignedByProperty.description.includes(
  ...
  it('should handle circular references in array types', () => {
    ...
    assert.strictEqual(
      userResult.schema.type,
      'object',
      'User schema should be object type'
    )
    assert.strictEqual(
      roleResult.schema.type,
      'object',
      'Role schema should be object type'
    )

    // Verify that properties exist without infinite recursion
    assert.ok(
      userResult.schema.properties.role,
      'User should have role property'
    )
    assert.ok(
      roleResult.schema.properties.assignedBy,
      'Role should have assignedBy property'
    )

    // The properties should use $ref or be simple object types, not infinitely nested
    assertRefOrObjectType(
      userResult.schema.properties.role,
      'CircularRole',
      'User role property'
    )
  ...
  it('should cache circular reference resolutions efficiently', () => {
    ...
    assert.deepStrictEqual(
      userResult1,
      userResult2,
      'User results should be identical when cached'
    )
    assert.deepStrictEqual(
      roleResult1,
      roleResult2,
      'Role results should be identical when cached'
    )

    // Both should still be valid object schemas
    assert.strictEqual(
      userResult1.schema.type,
      'object',
      'Cached user schema should be object type'
    )
    assert.strictEqual(
      roleResult1.schema.type,
      'object',
      'Cached role schema should be object type'
    )
  })
  ...
  it('should handle complex circular references with multiple classes - CircularOrganization', () => {
    ...
    assert.ok(
      result.schema.properties.defaultRole,
      'Should have defaultRole property'
    )
    assert.ok(result.schema.properties.members, 'Should have members property')
    assert.ok(
      result.schema.properties.availableRoles,
      'Should have availableRoles property'
    )
    assert.ok(
      result.schema.properties.parentOrganization,
      'Should have parentOrganization property'
    )
    assert.ok(
      result.schema.properties.childOrganizations,
      'Should have childOrganizations property'
    )

    // Verify that circular references are handled with $ref or object types
    assertRefOrObjectType(
      result.schema.properties.owner,
      'CircularUser',
      'Owner'
    )
    assertRefOrObjectType(
    ...
    assert.strictEqual(
      result.schema.properties.members.type,
      'array',
      'Members should be array type'
    )
    assert.strictEqual(
      result.schema.properties.availableRoles.type,
      'array',
      'AvailableRoles should be array type'
    )
    assertRefOrObjectType(
      result.schema.properties.parentOrganization,
      'CircularOrganization',
    ...
    assert.strictEqual(
      result.schema.properties.childOrganizations.type,
      'array',
      'ChildOrganizations should be array type'
    )
  })
  ...
  it('should handle self-referencing classes without infinite recursion', () => {
    ...
    // The parentOrganization should use $ref or be an object but not infinitely nested
    ...
    assert.strictEqual(
      childOrgsProperty.type,
      'array',
      'ChildOrganizations should be array type'
    )

    // Check that self-references don't create infinite nesting (only if not using $ref)
    if (!parentOrgProperty.$ref && parentOrgProperty.properties) {
      ...
      if (parentOrgProperty.properties.parentOrganization) {
        assertRefOrObjectType(
          parentOrgProperty.properties.parentOrganization,
          'CircularOrganization',
        ...
        assert.ok(
          !parentOrgProperty.properties.parentOrganization.properties
  ...
  it('should handle multi-level circular references between all three classes', () => {
    ...
    assert.strictEqual(
      orgResult.schema.type,
      'object',
      'Organization schema should be object type'
    )
    ...
    assert.ok(
      userResult.schema.properties.organization,
      'User should have organization property'
    )
    assert.ok(
      userResult.schema.properties.organizations,
      'User should have organizations property'
    )
    ...
    assert.ok(
      roleResult.schema.properties.organization,
      'Role should have organization property'
    )
    assert.ok(
      roleResult.schema.properties.usersWithRole,
      'Role should have usersWithRole property'
    )

    // Organization should have references to User, Role, and itself
    assert.ok(
      orgResult.schema.properties.owner,
      'Organization should have owner property'
    )
    assert.ok(
      orgResult.schema.properties.defaultRole,
      'Organization should have defaultRole property'
    )
    assert.ok(
      orgResult.schema.properties.parentOrganization,
      'Organization should have parentOrganization property'
    )
    assert.ok(
      orgResult.schema.properties.childOrganizations,
      'Organization should have childOrganizations property'
    )
  })
  ...
  it('should handle arrays of circular references efficiently', () => {
    ...
    assert.strictEqual(
      membersProperty.type,
      'array',
      'Members should be array type'
    )
    assert.strictEqual(
      availableRolesProperty.type,
      'array',
      'AvailableRoles should be array type'
    )
    assert.strictEqual(
    ...
    // Array items should use $ref or be objects (not infinitely nested)
    if (membersProperty.items) {
      assertRefOrObjectType(
        membersProperty.items,
        'CircularUser',
        'Members items'
      )
    }
    ...
    if (childOrgsProperty.items) {
      assertRefOrObjectType(
        childOrgsProperty.items,
        'CircularOrganization',
        'ChildOrganizations items'
      )
    }
  })
  ...
  it('should handle deep circular reference chains (A->B->C->D->A)', () => {
    ...
    assert.strictEqual(
      resultA.schema.type,
      'object',
      'DeepCircularA schema should be object type'
    )
    assert.strictEqual(
      resultB.schema.type,
      'object',
      'DeepCircularB schema should be object type'
    )
    assert.strictEqual(
      resultC.schema.type,
      'object',
      'DeepCircularC schema should be object type'
    )
    assert.strictEqual(
      resultD.schema.type,
      'object',
      'DeepCircularD schema should be object type'
    )

    // Check that each class has its reference property
    assert.ok(
      resultA.schema.properties.reference,
      'DeepCircularA should have reference property'
    )
    assert.ok(
      resultB.schema.properties.reference,
      'DeepCircularB should have reference property'
    )
    assert.ok(
      resultC.schema.properties.reference,
      'DeepCircularC should have reference property'
    )
    assert.ok(
      resultD.schema.properties.reference,
      'DeepCircularD should have reference property'
    )

    // All references should use $ref or be object types (not infinitely nested)
    assertRefOrObjectType(
      resultA.schema.properties.reference,
      'DeepCircularB',
      'A->B reference'
  ...
  it('should handle complex object with multiple circular references', () => {
    ...
    assert.ok(
      resultD.schema.properties.allReferences,
      'Should have allReferences property'
    )
    assert.ok(
      resultD.schema.properties.arrayReferences,
      'Should have arrayReferences property'
    )

    // allReferences should be an object with nested properties
    const allRefsProperty = resultD.schema.properties.allReferences
    assert.strictEqual(
      allRefsProperty.type,
      'object',
      'allReferences should be object type'
    )

    // arrayReferences should be an array
    const arrayRefsProperty = resultD.schema.properties.arrayReferences
    assert.strictEqual(
      arrayRefsProperty.type,
      'array',
      'arrayReferences should be array type'
    )
    ...
    if (allRefsProperty.properties) {
      ...
      Object.values(allRefsProperty.properties).forEach((prop: any) => {
        assert.strictEqual(
          prop.type,
          'object',
          'Each reference in allReferences should be object type'
        )
      })
    }
  })

  it('should use $ref pattern for circular references (OpenAPI 3.1 style)', () => {
    const transformer = SchemaTransformer.getInstance()

    const userResult = transformer.transform(CircularUser)

    // The role property should use $ref for circular reference
    const roleProperty = userResult.schema.properties.role

    if (roleProperty.$ref) {
      // If using $ref, it should be properly formatted
      assert.ok(
        roleProperty.$ref.includes('CircularRole'),
        '$ref should reference CircularRole'
      )
      ...
      // Fallback: should at least be an object type with circular reference detection
      assert.strictEqual(
        roleProperty.type,
        'object',
        'Should be object type if not using $ref'
      )
      if (roleProperty.description) {
        assert.ok(
          roleProperty.description.includes('circular reference detected'),
          'Should indicate circular reference in description'
        )
      }
    }
  })
  ...
  it('should handle performance with multiple deep circular transformations', () => {
    ...
    assert.ok(
      duration < 1000,
      `Performance test failed: took ${duration}ms for 20 transformations`
    )

    // All should still work correctly
    const finalResult = transformer.transform(DeepCircularD)
    assert.strictEqual(
      finalResult.schema.type,
      'object',
      'Final result should still be object type'
    )
  })
})
