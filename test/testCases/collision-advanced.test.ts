...
describe('Advanced Class Collision Handling', () => {
  /* it('should correctly identify throwing classes using metadata or other means', () => {
    ...
    assert.strictEqual(
      schemaA.properties?.['uniqueA']?.type,
      'string',
      'Should find uniqueA in ClassA'
    )
    assert.strictEqual(
      schemaA.properties?.['uniqueB'],
      undefined,
      'Should not find uniqueB in ClassA'
    )

    const { schema: schemaB } = transform(ClassB)
    assert.strictEqual(
      schemaB.properties?.['uniqueB']?.type,
      'boolean',
      'Should find uniqueB in ClassB'
    )
    assert.strictEqual(
      schemaB.properties?.['uniqueA'],
      undefined,
  ...
  it('should correctly identify classes with different array content types', () => {
    ...
    assert.strictEqual(
      schemaString.properties?.['tags'].items?.type,
      'string',
      'Should identify string array'
    )

    const { schema: schemaNumber } = transform(NumberArrayClass)
    assert.strictEqual(
      schemaNumber.properties?.['tags'].items?.type,
      'number',
      'Should identify number array'
    )
  })
})
